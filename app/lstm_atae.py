...
class AspectBasedSentimentAnalysis:
    ...
    def test(self, test_dataset):
        ...
        # Convert tokenized sentences and aspect terms to embeddings
        sentence_embeddings = np.array([np.mean([self.word2vec_model.wv[token] for token in word_tokenize(sentence.lower()) if token in self.word2vec_model.wv], axis=0) for sentence in test_dataset['input_text']])
